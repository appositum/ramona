defmodule Ramona.Commands.Basic do
  ...
  Cogs.def say(str) do
    ...
    if message.author.id != Cache.user().id and
         !Utils.invite_match?(message.content) and
         !String.contains?(message.content, patt) do
      str
      |> Utils.escape_prefix()
      |> Cogs.say()
    end
  ...
  Cogs.def sayin(s) do
    if message.author.id != Cache.user().id and
         !Utils.invite_match?(message.content) do
  ...
  Cogs.def mixcolors(hex1, hex2) do
    with {:ok, color1} <- Utils.parse_color(hex1, true, true),
         {:ok, color2} <- Utils.parse_color(hex2, true, true) do
      hash = Utils.gen_hash()
  ...
  Cogs.def polar(arg) do
    file =
      case arg do
        ...
        "sunglasses" -> "polar_bear_sunglasses.gif"
        "swim" -> "polar_bear_swim.gif"
        _ -> "polar_bear.gif"
      end
  ...
  Cogs.def reverse(msg) do
    ...
    if message.author.id != Cache.user().id and
         not String.contains?(message.content, patt) and
         !Utils.invite_match?(message.content) and
         !Utils.invite_match?(String.reverse(message.content)) do
      msg
      |> String.reverse()
      |> Utils.escape_prefix()
      |> Cogs.say()
  ...
  Cogs.def top(_) do
    Cogs.say(
      "<:topkk1:486252311279304714><:topkk2:486252311325310976><:topkk3:486252311312859137><:topkk4:486252311459659816><:topkk5:486252311434231810>"
    )
  end
